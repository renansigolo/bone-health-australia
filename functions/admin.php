...
function joints_rss_dashboard_widget() {
	if ( function_exists( 'fetch_feed' ) ) {
		...
		$feed  = fetch_feed( 'https://jointswp.com/feed/rss/' );        // specify the source feed.
		$limit = $feed->get_item_quantity( 5 );                      // specify number of items.
		$items = $feed->get_items( 0, $limit );                      // create an array of items.
	}
	if ( $limit == 0 ) {
		echo '<div>' . __( 'The RSS Feed is either empty or unavailable.', 'bone_health' ) . '</div>';
	}   // fallback message.
	else {
		foreach ( $items as $item ) { ?>
			...
			<h4 style="margin-bottom: 0;">
				...
				" title="
				<?php
				echo mysql2date( __( 'j F Y @ g:i a', 'bone_health' ), $item->get_date( 'Y-m-d H:i:s' ) );
				?>
				"
				   target="_blank">
					<?php
...
function joints_custom_dashboard_widgets() {
	wp_add_dashboard_widget(
		...
		__( 'Custom RSS Feed (Customize in admin.php)', 'bone_health' ),
		'joints_rss_dashboard_widget'
	);
	/*
...
function joints_custom_admin_footer() {
	_e(
		'<span id="footer-thankyou">Developed by <a href="#" target="_blank">Your Site Name</a></span>.',
		'bone_health'
	);
}

...
function joints_excerpt_more( $more ) {
	...
	return '<a class="excerpt-read-more" href="' . get_permalink( $post->ID ) . '" title="' . __(
		'Read',
		'bone_health'
	) . get_the_title( $post->ID ) . '">' . __( '... Read more &raquo;', 'bone_health' ) . '</a>';
}
...
function joints_get_the_author_posts_link() {
	...
	$link = sprintf(
		...
		esc_attr( sprintf( __( 'Posts by %s', 'bone_health' ), get_the_author() ) ),

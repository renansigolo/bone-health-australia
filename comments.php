...
<div id="comments" class="comments-area">
	...
	if ( have_comments() ) :
		...
		if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through?
			...
			<nav id="comment-nav-above" class="navigation comment-navigation" role="navigation">
				...
				<?php
					esc_html_e( 'Comment navigation', 'bone_health' );
				?>
					</h2>
				<div class="nav-links">
					...
					<?php
						previous_comments_link( esc_html__( 'Older Comments', 'bone_health' ) );
					?>
						</div>
					<div class="nav-next">
					<?php
						next_comments_link( esc_html__( 'Newer Comments', 'bone_health' ) );
					?>
						</div>
		...
		<ol class="commentlist">
			<?php
			wp_list_comments( 'type=comment&callback=joints_comments' );
			?>
		</ol>
	...
	if ( ! comments_open() && '0' !== get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		...
		<p class="no-comments">
		<?php
			esc_html_e( 'Comments are closed.', 'bone_health' );
		?>
			</p>
		<?php
	endif;
